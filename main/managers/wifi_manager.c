...
esp_err_t captive_portal_redirect_handler(httpd_req_t *req) {
    ...
    const char *uri = req->uri;
    // Samsung-specific fix: Return 200 OK for /generate_204 only for Samsung devices
    ...
    if (strcmp(uri, "/generate_204") == 0 || strcmp(uri, "/gen_204") == 0) {
        // Check if this is a Samsung device by User-Agent
        size_t ua_len = httpd_req_get_hdr_value_len(req, "User-Agent") + 1;
        if (ua_len > 1) {
            char *ua = malloc(ua_len);
            if (ua) {
                if (httpd_req_get_hdr_value_str(req, "User-Agent", ua, ua_len) == ESP_OK) {
                    // Check for Samsung device indicators in User-Agent
                    bool is_samsung = (strstr(ua, "SM-") != NULL) ||           // Samsung model numbers
                                     (strstr(ua, "SAMSUNG") != NULL) ||         // Samsung brand
                                     (strstr(ua, "GT-") != NULL) ||            // Older Samsung models
                                     (strstr(ua, "SCH-") != NULL) ||           // Sprint Samsung models
                                     (strstr(ua, "SPH-") != NULL) ||           // Sprint Samsung models
                                     (strstr(ua, "SGH-") != NULL) ||           // Samsung GSM models
                                     (strstr(ua, "Galaxy") != NULL);           // Galaxy series
                    ...
                    if (is_samsung) {
                        ...
                        httpd_resp_set_type(req, "text/html");
                        httpd_resp_send(req, NULL, 0);
                        ESP_LOGI(TAG, "Samsung captive portal fix: Returning 200 OK for %s (User-Agent: %s)", uri, ua);
                        free(ua);
                        return ESP_OK;
                    } else {
                        ESP_LOGI(TAG, "Non-Samsung device detected for %s, proceeding with normal captive portal logic (User-Agent: %s)", uri, ua);
                    }
                }
                free(ua);
            }
        }
        // If not Samsung or User-Agent check failed, fall through to normal logic
    }
